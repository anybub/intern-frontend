...
const CreateElectionDashboard = ({ id }: Props) => {
  const [show, setShow] = useState(id ? 1 : 0);
  const [electionId, setElectionId] = useState<string | null>(id);
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 h-[100vh] w-full">
      ...
      <div className="col-span-2">
        {show === 0 && electionId===null && (
          <CreateElection setElectionId={setElectionId} />
        )}
        {show === 1 && electionId && <AddCandidate electionId={electionId}/>}
        {show === 2 && electionId && <AddVoters />}
      </div>
    </div>
  );
};
