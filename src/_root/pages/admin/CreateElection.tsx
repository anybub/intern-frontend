...
import { ElectionCreatedEvent } from "@/typechain-types/ElectionManager";
interface CreateElectionProps {
    setElectionId: (id: string) => void;
    setBlockChainElectionId: (id: number) => void;
}
const CreateElection = ({
    setElectionId,
    setBlockChainElectionId,
}: CreateElectionProps) => {
    ...
    const makeElection = useMutation({
        mutationFn: async (values: z.infer<typeof ElectionValidation>) => {
            const res = await fetch(
                "http://localhost:5000/api/v1/election/createElection",
                {
                    method: "POST",
                    headers: {
    ...
    async function onSubmit(values: z.infer<typeof ElectionValidation>) {
        try {
            const tx = await contract?.createElection(values.name as string);
            await tx?.wait();
            const filter = contract?.filters[
                "ElectionCreated(uint256,string,address)"
            ] as ElectionCreatedEvent.Event;
            contract?.on(filter, (electionCount, _name, creator) => {
                console.log(electionCount, _name, creator);
                setBlockChainElectionId(Number(electionCount));
            });
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="flex flex-col">
                                    ...
                                    <FormDescription>
                                        Election will start at 12:00 AM on the
                                        selected date
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
